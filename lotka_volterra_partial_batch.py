...
class Bivariate_Normal():
    ...
    def __init__(self, mu, chol):
        ...
        # self.mu = mu
        self.chol = chol # (p*(T-1), 2, 2)
        # + tf.expand_dims(tf.eye(2) * 1e-6, 0) 
...
class VI_SSM():
    ...
    def __init__(self, obs, obs_bin, time_till, x0_mean, x0_std, theta_dist, priors, dt, T, p_val, kernel_len, batch_dims, network_dims, target_dims, no_flows, feat_window, learn_rate = 1e-3, pre_train=True):
        ...
        self.theta = theta_dist.sample(p_val) #(p,4)
        self.theta_log_prob = theta_dist.log_prob(self.theta)
        print('theta log prob:', self.theta_log_prob.shape)
        self.priors = priors
        self.dt = dt
        self.p_val = p_val
        ...
        print('bin_feats:', self.bin_feats.shape)
        print('time_till:', self.time_till.shape)
        print('time_pad:', self.time_pad.shape)
        print('obs_pad_store:', len(self.obs_pad_store))
        print('obs_pad_store[0]:', self.obs_pad_store[0].shape)
    ...
    def _ELBO(self):
        ...
        self.sde_mu = self.sde_drift + self.xt_plus_dt ## (p*(T-1), 2)
        ...
        SDE_log_prob = tf.reduce_sum(tf.reshape(
            self.bvn.log_prob(self.lf_sample_flat, 
                              bijector_chain=None
                              ), [self.p_val, -1]), 1) # (p,)
        ...
        #all have dimensions of (p,)
        ###!!check (self.target_dims / self.batch_dims)
    ...
    def build_flow(self):
        ...
        # lf_sample_neg: (p_val=50, (u-v)=2, M+1=51)
    ...
    def train(self, tensorboard_path, save_path, series_idx=None):
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            # print('batch_select:', batch_select)
    ...
    def save_paths(self, PATH_obs):
        ...
        for ii, index_temp in tqdm(enumerate(np.arange(0, self.batch_dims * self.p_val, self.batch_dims))):
            ...
            # print('time_feats_feed:', time_feats_feed.shape)
            ...
            # print('mask_feed:', mask_feed.shape)
            # print('self.mask', self.mask)
            # print('shift_feed:', shift_feed.shape)
            # print('self.shift:', self.shift)
            # print('bin_feed:', bin_feed.shape)
            # print('self.bin_feed:', self.bin_feed)
            # print('time_feats:', self.time_feats)
            # print('self.time_feats:', self.time_feats)
...
# hyperparams
p_val = 3 #3  #number of random batches to be picked from time series
kernel_len = 20
dt = 0.2
T = 30
...
var_model.save_paths(f'locally_variant/LV_obs_paths_series.txt')
print('Paths saved')
print('Training...')
var_model.train(tensorboard_path='locally_variant/train/',
                save_path=f'model_saves/LV_model_series_{batch_dims}_3.ckpt', series_idx=None)
