...
class Bivariate_Normal():
    ...
    def __init__(self, mu, chol):
        ...
        self.chol = chol + tf.expand_dims(tf.eye(2) * 1e-3, 0) # (p*(T-1), 2, 2)
        self.det = tf.reduce_prod(tf.matrix_diag_part(chol), axis=1) ** 2
        cov_matrix = chol @ tf.transpose(chol, [0, 2, 1])
        
        # invalid_mask = tf.logical_or(tf.is_nan(self.chol), tf.is_inf(self.chol))
        # tf.compat.v1.debugging.check_numerics(self.chol, "Cholesky matrix contains NaN/Inf")
        
        def compute_pseudo_inverse(cov_matrix, det):
            print(f"Warning: Covariance matrix is singular or nearly singular.\n det={det}\n")
            ...
            if method1:
                s_inv = tf.where(s > 1e-3, 1.0 / s, tf.zeros_like(s))
                print('s_inv.shape', s_inv.shape)
                print('s_inv diag:', tf.linalg.diag(s_inv).shape)
                cov_inv = tf.matmul(v, tf.matmul(tf.linalg.diag(s_inv), tf.transpose(u, perm=[0, 2, 1])))
            if not(method1):
                s_update = s + 1e-3 * tf.eye(2)
                s_inv = 1.0 / s_update
                cov_inv = tf.matmul(v, tf.matmul(tf.linalg.diag(s_inv), tf.transpose(u, perm=[0, 2, 1])))
                det = tf.reduce_prod(s_update)
            # tf.compat.v1.debugging.check_numerics(cov_inv, "Inverse covariance matrix contains NaN/Inf")  
            return cov_inv, det       
             
                            
        determinant_min = tf.reduce_min(tf.abs(self.det))
        
        self.cov_inv, self.det = tf.cond(
            determinant_min < 1e-8,  # Condition (Tensor)
            lambda: compute_pseudo_inverse(cov_matrix, self.det),  # If True: Use pseudo-inverse
            lambda: (tf.matrix_inverse(cov_matrix), self.det)  # If False: Use regular inverse
        )
...
class VI_SSM():
    ...
    def __init__(self, obs, obs_bin, time_till, x0_mean, x0_std, theta_dist, fix_theta, sparse, priors, dt, T, p_val, kernel_len, batch_dims, network_dims, target_dims, no_flows, feat_window, learn_rate = 1e-3, pre_train=True):
        ...
        _bin_feats = np.float32(np.concatenate(
        ...
        self.time_pad = np.broadcast_to(_time_pad.reshape(1, 364, 1) , (self.num_batches, len(_time_pad), 1)) #(1024, 364, 1)
...
try:
    ...
    f3.close()
    print('Data loaded')
    # obs = obs[:,bs_idx*batch_dims:(bs_idx+1)*batch_dims] #(2, p*T)
    # obs_bin = obs_bin[:,bs_idx*batch_dims:(bs_idx+1)*batch_dims]
    # time_till = time_till[:,bs_idx*batch_dims:(bs_idx+1)*batch_dims]

    # obs = obs[:,2*batch_dims:3*batch_dims] #(2, p*T)
    # obs_bin = obs_bin[:,2*batch_dims:3*batch_dims]
    # time_till = time_till[:,2*batch_dims:3*batch_dims]
    
    # obs = obs[:,:6*batch_dims] #(2, p*T)
    # obs_bin = obs_bin[:,:6*batch_dims]
    # time_till = time_till[:,:6*batch_dims]
