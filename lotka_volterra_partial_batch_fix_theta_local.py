        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            if epoch == num_epochs - 1:
                lf_sample_val = sess.run([self.lf_sample], feed_dict={
                    self.time_feats: time_feats_feed, self.mask: mask_feed, self.shift: shift_feed, self.bin_feed: bin_feed, self.diffusivity: [0.0]})
                np.save(f'dat/our_files/fix_theta/out_dense/{series_idx_str}learned_lf_sample.npy', lf_sample_val)
                for i in range(100):
                    ...
                    lf_sample_val = sess.run(lf_sample, feed_dict={
                        self.time_feats: time_feats_feed, self.mask: mask_feed, self.shift: shift_feed, self.bin_feed: bin_feed, self.diffusivity: [0.0]})
                    np.save(f'dat/our_files/fix_theta/out_dense/{series_idx_str}learned_lf_sample_%d.npy' % i, lf_sample_val)
...
for bs_idx in tqdm(range(150)):
    try:
        ...
        with tf.Session() as sess:  # or InteractiveSession if needed
            ...
            var_model.save_paths(f'locally_variant/LV_obs_paths_series_dense_{bs_idx}.txt')
            print('Paths saved')
            print('Training...')
            var_model.train(tensorboard_path='locally_variant/train_fix_theta_dense',
                            save_path=f'model_saves/LV_model_dense_fix_theta_series_{batch_dims}_3_{bs_idx}.ckpt',
