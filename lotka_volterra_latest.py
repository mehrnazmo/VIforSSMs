...
class IAF():
    ...
    def __init__(self, network_dims, theta, ts_feats, feat_dims = 10):
        self.network_dims = network_dims #[50]*5
        self.num_layers = len(network_dims) #5
        self.theta = theta #(p,4)
        self.ts_feats = ts_feats #(p, 162, l+3)
        self.feat_dims = feat_dims #162 - 1 - i * 20 for i in range(3)
    ...
    def __init__(self, obs, obs_bin, time_till, x0_mean, x0_std, theta_dist, fix_theta, sparse, priors, dt, T, p_val, kernel_len, batch_dims, network_dims, target_dims, no_flows, feat_window, learn_rate = 1e-3, pre_train=True):
        ...
        self.bin_feats = np.float32(np.concatenate(
            (np.zeros(self.no_flows * self.kernel_len + self.flow_dims), 
             np.ones(self.target_dims * self.flow_dims)), axis=0))  #62+2pT
        
        self.mask_vals = np.concatenate(
            (np.zeros((2, 1)), np.ones((self.flow_dims, self.target_dims))), axis=1) #(2,p(T+1))
        self.shift_vals = np.concatenate(
            (np.expand_dims(x0_mean, 1), np.zeros((self.flow_dims, self.target_dims))), axis=1)#(2,p(T+1))
...
for bs_idx in tqdm(range(150)):
    try:
        print('\nstarting series:', bs_idx)
        ...
        with tf.Session() as sess:  # or InteractiveSession if needed
            ...
            var_model.save_paths(f'{out_dir}/lf_sample_series_{bs_idx}.txt')
            # print('Paths saved')
            # print('Training...')
            # np.savetxt('/home/b2028663/scripts/arf/locally_variant/local_post.txt', sess.run(theta_dist.sample([100000])))
            var_model.train(tensorboard_path=f'{out_dir}/tf_series_{bs_idx}',
                            save_path=f'{out_dir}/LV_model_series_{bs_idx}.ckpt',
                            num_epochs=num_epochs, pre_train_epochs=pre_train_epochs, series_idx=bs_idx)
        print('Success in series:', bs_idx)
    except Exception as e:
        print('\nError:', e)
        print()
        print(traceback.print_exc())
        print()
        print('Error in series:', bs_idx)
        continue
    print('Finished series:', bs_idx)
